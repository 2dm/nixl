...
# noinspection PyUnresolvedReferences
from .nixl_ep_cpp import EventHandle, Config
...
class Buffer:
    ...
    Attributes:
        ...
        num_nvl_bytes: the buffer size for NVLink communication in high-throughput mode.
        num_rdma_bytes: the buffer size for RDMA communication.
        runtime: the C++ runtime.
        low_latency_mode: whether the buffer is in low-latency mode.
    """
    ...
    def __init__(self, group: Optional[dist.ProcessGroup] = None,
        ...
        Arguments:
            ...
            enable_shrink: whether to enable shrink mode for dynamic rank masking.
    ...
    def get_dispatch_config(num_ranks: int) -> Config:
        """
        Get a recommended dispatch config for high-throughput mode.
    ...
    def get_combine_config(num_ranks: int) -> Config:
        """
        Get a recommended combine config for high-throughput mode.
    ...
    def get_dispatch_layout(self, topk_idx: torch.Tensor, num_experts: int,
        ...
        """
        Calculate the layout required for later communication (high-throughput mode).
    ...
    def dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        Requires the ranks in a node to be visible via NVLink, and the ranks with the same GPU
            index across nodes to be visible via RDMA.
        ...
        # Internode dispatch requires num_rdma_ranks > 1
        if self.runtime.get_num_rdma_ranks() <= 1:
            raise RuntimeError(
                "High-throughput dispatch requires internode setup (num_rdma_ranks > 1). "
                "Please use low_latency_dispatch() for single-node configurations."
            )
    ...
    def combine(self, x: torch.Tensor, handle: Tuple,
        ...
        Arguments:
            ...
            bias: optional bias tensor(s) to add to the result.
            config: the performance tuning config.
            previous_event: the event to wait before actually executing the kernel.
            async_finish: the current stream will not wait for the communication kernels to be finished if set.
            allocate_on_comm_stream: control whether all the allocated tensors' ownership to be on the communication stream.
        ...
        # Internode combine requires num_rdma_ranks > 1
        if self.runtime.get_num_rdma_ranks() <= 1:
            raise RuntimeError(
                "High-throughput combine requires internode setup (num_rdma_ranks > 1). "
                "Please use low_latency_combine() for single-node configurations."
            )
    ...
    def low_latency_dispatch(self, x: torch.Tensor, topk_idx: torch.Tensor,
        ...
        This kernel requires all the ranks to be visible via RDMA.
    ...
    def low_latency_combine(self, x: torch.Tensor, topk_idx: torch.Tensor, topk_weights: torch.Tensor,
        ...
        Arguments:
            ...
            handle: the communication handle given by the `low_latency_dispatch` function.
    ...
    def low_latency_update_mask_buffer(self, rank_to_mask: int, mask: bool = False):
        """
        Mask (unmask) a rank during communication (dispatch, combine, and clean) in low-latency mode.
    ...
    def low_latency_query_mask_buffer(self, mask_status: torch.Tensor):
        """
        Query the mask status of all ranks in low-latency mode.
    ...
    def low_latency_clean_mask_buffer(self):
        """
        Clean the mask buffer in low-latency mode.
        """
    ...
    def update_memory_buffers(self, num_ranks: int, num_experts_per_rank: int,
                              num_nvl_bytes: int, num_rdma_bytes: int):
        """
        ...
        Arguments:
            ...
            num_nvl_bytes: the buffer size for NVLink communication (used in high-throughput mode).
            num_rdma_bytes: the buffer size for RDMA communication.
        """
    ...
    def low_latency_barrier(self) -> None:
        """
        Barrier all active ranks in low-latency mode.
