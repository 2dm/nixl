...
def test_main(args: argparse.Namespace, num_sms: int,
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_e4m3):
                for with_topk in (False, True):
                    ...
                    recv_x, recv_topk_idx, recv_topk_weights, recv_num_tokens_per_expert_list, handle, event = buffer.internode_dispatch(**dispatch_args)
    ...
    for current_x in (x_e4m3, x):
        ...
        for nvl_chunk_size in range(4, 45, 4):
            for rdma_chunk_size in range(4, 33, 4):
                ...
                t, notify_t = bench_kineto(lambda: buffer.internode_dispatch(**tune_args), ('dispatch', 'notify'))
                if t < best_time:
    ...
    recv_x, _, _, _, handle, _ = buffer.internode_dispatch(**dispatch_args)
    ...
    for nvl_chunk_size in range(1, 8, 1):
        for rdma_chunk_size in range(12 if num_nodes == 2 else 8, 33, 4):
            ...
            t, notify_t = bench_kineto(lambda: buffer.internode_combine(**tune_args), ('combine', 'notify'))
            if local_rank == 0:
