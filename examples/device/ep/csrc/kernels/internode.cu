...
__forceinline__ __device__ void nixl_barrier(nixl_ep::gpu_nixl_ctx nixl_ctx, int num_channels) {
    ...
    for (int j=0; j<num_channels; j++) {
        for (int i = 0; i < nixl_ctx.num_rdma_ranks; i++) {
            ...
            // Use PostSignalXferReq to increment remote barrier counter by 1
            nixl_status_t status = nixlGpuPostSignalXferReq<nixl_gpu_level_t::THREAD>(
                remote_barrier_handle, 0, 1, 0, j, true, &request_status);
    ...
    if (sm_id == 0) {
        ...
        for (int i = warp_id; i < kNumRDMARanks; i += num_warps) {
            if (i != rdma_rank) {
                ...
                nixl_status_t status = nixlGpuPostSingleWriteXferReq<nixl_gpu_level_t::WARP>(
                    batch_req, 0, src_offset, dst_offset, msg_size, 0, true);
                EP_DEVICE_ASSERT(status == NIXL_IN_PROG);
            } else {
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (int dst_rdma_rank = warp_id; dst_rdma_rank < kNumRDMARanks; dst_rdma_rank += kNumDispatchRDMASenderWarps) {
            ...
            if (dst_rdma_rank != rdma_rank) {
                ...
                EP_DEVICE_ASSERT(nixlGpuPostSingleWriteXferReq<nixl_gpu_level_t::WARP>(
                                     batch_req, 0, src_offset, dst_offset, msg_size, channel_id, true) ==
                                 NIXL_IN_PROG);
        ...
        while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
            ...
            while (true) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < kNumRDMARanks) {
                    printf(
                        "NixlEP dispatch forwarder timeout (RDMA check), channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, src RDMA lane: %d, head: %d, tail: %d, expected: %d\n",
                        channel_id,
                        rdma_rank,
                        nvl_rank,
                        dst_nvl_rank,
        ...
        while (true) {
            ...
            if (min_head != std::numeric_limits<int>::max() and min_head >= last_head + num_max_rdma_chunked_send_tokens and
                ...
                if(lane_id == rdma_rank || dst_p2p_ptr != 0){
                    ...
                    nixl_status_t status = nixlGpuPostSignalXferReq<nixl_gpu_level_t::THREAD>(
                        batch_req, 0, min_head - last_head, head_counter_offset);
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < kNumRDMARanks) {
                    printf(
                        "NixlEP combine NVL sender timeout, channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, RDMA lane: %d, head: %d, tail: %d, start: %d, end: %d\n",
                        channel_id,
                        rdma_rank,
                        nvl_rank,
                        dst_nvl_rank,
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
                ...
                for (int token_idx = token_start_idx + sub_warp_id; token_idx < token_end_idx; token_idx += kNumWarpsPerForwarder) {
                    ...
                    while (cached_nvl_channel_tail_idx <= expected_head) {
                        ...
                        if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < NUM_MAX_NVL_PEERS) {
                            printf(
                                "NixlEP combine forwarder (NVL check) timeout, channel: %d, RDMA: %d, nvl: %d, src NVL: %d, dst RDMA: %d, tail: %d, waiting: %d, total: %d, sub: %d, large: %d, expected: %d\n",
                                channel_id,
                                rdma_rank,
                                nvl_rank,
                                lane_id,
            ...
            for (int64_t token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumRDMAReceivers) {
                ...
                while (cached_channel_tail_idx <= expected_head) {
                    ...
                    if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                        printf(
                            "NixlEP combine RDMA receiver timeout, channel: %d, RDMA: %d, nvl: %d, src RDMA: %d, tail: %d, waiting: %ld, expect: %d\n",
                            channel_id,
                            rdma_rank,
                            nvl_rank,
                            lane_id,
