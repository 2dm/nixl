...
void Buffer::init(int num_ranks, int num_experts_per_rank, int64_t num_nvl_bytes, int64_t num_rdma_bytes)
{
    ...
    // Allocate internode barrier counter (for high-throughput mode)
...
void Buffer::destroy() {
    ...
    // Free internode barrier epoch counter if allocated
    if (nixl_ctx && nixl_ctx->gpu.last_barrier_counter) {
        cudaFree(nixl_ctx->gpu.last_barrier_counter);
        nixl_ctx->gpu.last_barrier_counter = nullptr;
    }
...
Buffer::internode_dispatch(const torch::Tensor& x, const std::optional<torch::Tensor>& x_scales,
    ...
    if (cached_mode) {
        ...
        internode::cached_notify(hidden_int4, num_scales, num_topk, num_topk,
                                 ...
                                 num_nvl_bytes, true, low_latency_mode, nixl_ctx->gpu);
    } else {
        ...
        *moe_recv_counter = -1, *moe_recv_rdma_counter = -1;
        ...
        internode::notify_dispatch(num_tokens_per_rank->data_ptr<int>(), moe_recv_counter_mapped, num_ranks,
                                   ...
                                   barrier_signal_ptrs_gpu, rank, comm_stream,
                                   config.get_rdma_buffer_size_hint(hidden_int4 * sizeof(int4), num_ranks),
                                   num_nvl_bytes, low_latency_mode, nixl_ctx->gpu);
    ...
    internode::dispatch(recv_x.data_ptr(), recv_x_scales_ptr, recv_topk_idx_ptr, recv_topk_weights_ptr,
                        ...
                        comm_stream, num_channels, low_latency_mode, nixl_ctx->gpu);
...
Buffer::internode_combine(const torch::Tensor& x, const std::optional<torch::Tensor>& topk_weights,
    ...
    internode::cached_notify(hidden_int4, 0, 0, num_topk,
                             ...
                             barrier_signal_ptrs_gpu, rank, comm_stream,
                             config.get_rdma_buffer_size_hint(hidden_int4 * sizeof(int4), num_ranks),
                             num_nvl_bytes, false, low_latency_mode, nixl_ctx->gpu);
    ...
    internode::combine(at::cuda::ScalarTypeToCudaDataType(x.scalar_type()),
                       ...
                       rdma_buffer_ptr, config.num_max_rdma_chunked_send_tokens, config.num_max_rdma_chunked_recv_tokens,
                       buffer_ptrs_gpu, config.num_max_nvl_chunked_send_tokens, config.num_max_nvl_chunked_recv_tokens,
                       rank, num_ranks, comm_stream, num_channels, low_latency_mode, nixl_ctx->gpu);
...
void Buffer::_nixl_ep_gpu_ctx_update() {
    ...
    nixl_ctx->gpu.num_rdma_ranks = num_rdma_ranks;
    nixl_ctx->gpu.rank = rank;

    /* Initialize internode barrier counters for high-throughput mode */
    if (!low_latency_mode) {
        // local_barrier_counter_ptr points to our local_barrier_counter which remote peers write to
        nixl_ctx->gpu.local_barrier_counter_ptr = local_barrier_counter;

        // last_barrier_counter tracks barrier epochs (allocate once)
        if (nixl_ctx->gpu.last_barrier_counter == nullptr) {
            CUDA_CHECK(cudaMalloc(&nixl_ctx->gpu.last_barrier_counter, sizeof(uint64_t)));
            uint64_t zero = 0;
            CUDA_CHECK(cudaMemcpy(nixl_ctx->gpu.last_barrier_counter, &zero, sizeof(uint64_t), cudaMemcpyHostToDevice));
        }
    }
}
